...
// Função de impressão usando aritmética de ponteiros (mantida da atividade anterior)
void imprimir_produtos(Produto *ptr, int quantidade) {
    printf("\n--- RELATORIO DE ESTOQUE DINAMICO ---\n");
    for (int i = 0; i < quantidade; i++) {
        // Navegando na memória via aritmética de ponteiros
...
int main() {
    int quantidade;
    Produto *estoque = NULL; // Ponteiro que apontará para o nosso array dinâmico
    ...
    // 5. OBRIGATÓRIO: Libera a memória de volta para o sistema operacional
    free(estoque);
    estoque = NULL; // Evita que o ponteiro fique "solto" apontando para lixo (Dangling Pointer)
